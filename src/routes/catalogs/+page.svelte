<script lang="ts">
  ...
  import { CatalogTable, ExcelExporter, ExcelMultiSheetExporter } from "$lib/components";
  ...
  const operationDetailsColumns = [
    ...
    { key: "category", label: "Categoría", type: "text" as const },
    {
      ...
      badgeConfig: {
        ...
        } as Record<string, { text: string; class: string }>,
      },
    },
    { 
      key: "status", 
  ...
  async function handleTabChange(newTab: string) {
    ...
    if (!loadedCatalogs[newTab as keyof typeof loadedCatalogs]) {
      ...
      // Si ya está cargado, solo aplicar filtros
      applyFilters();
    }
  }
  ...
  function applyFilters() {
    ...
    if (filters.search) {
      data = data.filter((item) => {
        ...
        return Object.values(item).some((value) => value && value.toString().toLowerCase().includes(searchTerm));
      });
    }
     ...
     data.sort((a, b) => {
       let aValue = a.name || a.razonSocial || "";
       let bValue = b.name || b.razonSocial || "";
  ...
  // Obtener columnas del tab actual
  function getCurrentColumns() {
    switch (activeTab) {
      case "empresas":
        return empresasColumns;
      case "stands":
        return standsColumns;
      case "responsible-persons":
        return responsiblePersonsColumns;
      case "operation-details":
        return operationDetailsColumns;
      default:
        return [];
    }
  }

  // Función para obtener todos los catálogos para exportación múltiple
  function getAllCatalogsForExport() {
    return [
      {
        name: "Empresas",
        data: empresas,
      ...
      if (!newItem.name.trim()) {
       ...
       if (activeTab === "empresas") {
         response = await fetch("/api/catalogs/companies", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ razonSocial: newItem.name, ruc: newItem.location, status: "active" }),
         });
       } else if (activeTab === "stands") {
         response = await fetch("/api/catalogs/stands", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ name: newItem.name, location: newItem.location, status: "active" }),
         });
       } else if (activeTab === "responsible-persons") {
         response = await fetch("/api/catalogs/responsible-persons", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ name: newItem.name, email: newItem.email, phone: newItem.phone, status: "active" }),
         });
       } else if (activeTab === "operation-details") {
         response = await fetch("/api/catalogs/operation-details", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ name: newItem.name, type: newItem.type, category: newItem.category, status: "active" }),
         });
       }

      if (response && response.ok) {
        showSuccessMessage("Elemento creado correctamente");
        showCreateForm = false;
        // Resetear formulario
    ...
    <div class="mb-8">
     ...
     <div class="mb-6 bg-white p-4 rounded-lg shadow-sm border">
       <div class="grid grid-cols-4 gap-4">
         ...
         <div class="col-span-2">
           ...
           <input
             ...
             class="form-control h-10 leading-5 w-full px-3 border border-gray-300 rounded-md
         ...
         <div class="flex items-end gap-2">
           <button
             ...
             class="form-control h-10 leading-5 flex-1 flex items-center justify-center
			 px-3 rounded-md text-sm font-medium
			 bg-gray-100 text-gray-700 border border-gray-300
			 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400 transition"
           >
             Limpiar
           </button>
           ...
           <button
			 ...
			 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500 transition"
           >
      ...
      <div class="mt-4 text-sm text-gray-600">
        Mostrando {filteredData.length} de {activeTab === "empresas"
          ? empresas.length
          : activeTab === "stands"
            ? stands.length
            : activeTab === "responsible-persons"
              ? responsiblePersons.length
              : operationDetails.length} elementos
      </div>
    </div>
        ...
        <nav class="-mb-px flex space-x-8">
          <button
            ...
            class="py-2 px-1 border-b-2 font-medium text-sm
						{activeTab === 'empresas' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          >
            Empresas
          </button>
          <button
            ...
            class="py-2 px-1 border-b-2 font-medium text-sm
						{activeTab === 'stands' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          >
            Stands
          </button>
          <button
            ...
            class="py-2 px-1 border-b-2 font-medium text-sm
						{activeTab === 'responsible-persons' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          >
            Responsables
          </button>
          <button
            ...
            class="py-2 px-1 border-b-2 font-medium text-sm
						{activeTab === 'operation-details' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
          >
      ...
      <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-96">
          <h3 class="text-lg font-medium text-gray-900 mb-4">
           ...
           {#if activeTab === "empresas"}
             <div class="space-y-4">
               ...
               <div>
                 <label for="edit-empresa-status" class="block text-sm font-medium text-gray-700 mb-2">Estado</label>
                 <select
                   id="edit-empresa-status"
          ...
          {:else if activeTab === "stands"}
            <div class="space-y-4">
              ...
              <div>
                ...
                <select id="edit-stand-status" bind:value={editFormData.status} class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
                  <option value="active">Activo</option>
                  <option value="inactive">Inactivo</option>
                </select>
              </div>
           ...
           {:else if activeTab === "responsible-persons"}
             <div class="space-y-4">
               ...
               <div>
                 <label for="edit-person-status" class="block text-sm font-medium text-gray-700 mb-2">Estado</label>
                 <select
           ...
           {:else if activeTab === "operation-details"}
             <div class="space-y-4">
               ...
               <div>
                 ...
                 <select id="edit-detail-type" bind:value={editFormData.type} class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500">
               ...
               <div>
                 <label for="edit-detail-status" class="block text-sm font-medium text-gray-700 mb-2">Estado</label>
                 <select
                   id="edit-detail-status"
                   bind:value={editFormData.status}
                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
                 >
                   <option value="active">Activo</option>
          ...
          <div class="flex justify-end space-x-3 mt-6">
            ...
            <button onclick={updateItem} class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"> Actualizar </button>
          </div>
        </div>
      </div>
    {/if}
    ...
    {#if showCreateForm}
      <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-96">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <input
                  id="person-phone"
                  type="tel"
                  bind:value={newItem.phone}
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500"
                  placeholder="999888555"
                />
              </div>
            </div>
          {:else if activeTab === "operation-details"}
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button onclick={() => (showCreateForm = false)} class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50"> Cancelar </button>
            <button onclick={createItem} class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"> Crear </button>
          </div>
        </div>
      </div>
    {/if}
  ...
  {#if showDeleteModal && itemToDelete}
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-lg shadow-xl max-w-sm w-full">
        ...
        <!-- Contenido del modal -->
        <div class="p-6">
          <p class="text-sm text-gray-700">¿Estás seguro de que deseas eliminar este elemento? Esta acción no se puede deshacer.</p>
        </div>
